...
public class TagService {
	...
	public ArrayList<Tag> connectTagsWithZettel(ArrayList<Tag> tags, Zettel zettel) {
		...
		tags.forEach(
				tag -> LOG.info("\n HIER erhaltene tags die sollen ihre ID kriegen oder neu kriegen  !!!   nämlich:  "
						+ tag.getId() + " Anzahl Zettel: " + tag.getZettels().size()));

		ArrayList<Tag> newTags = tags.stream()
				.map(tag -> tagRepo.findByTagText(tag.getTagText()).orElse(new Tag(tag.getTagText())))
				.collect(Collectors.toCollection(ArrayList::new));
		newTags.forEach(tag -> LOG.info("\n HIER alle tags mit ID   !!!   nämlich:  " + tag.getId() + " Anzahl Zettel: "
				+ tag.getZettels().size()));
		newTags.forEach(tag -> tag.getZettels().add(zettel));
		newTags.forEach(tag -> System.out.println("\n HIER sollten alle verheiratet sein   !!!   nämlich:  "
				+ tag.getId() + " Anzahl Zettel: " + tag.getZettels().size()));
...
//		newTags.forEach(tag -> System.out.println("\n HIER sollten alle tags EINE id HABEN ä" + tag.getId() + tag.getTagText()));
		...
		// löschen der Tags, die in der DB & Zettel, aber nicht mehr in der neuen
		// Tagliste vorhanden sind (existed in old tag list, but not in new tag list)
