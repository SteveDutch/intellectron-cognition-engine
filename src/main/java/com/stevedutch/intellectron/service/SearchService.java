...
public class SearchService {
	 ...
	 * 
	 * @param name      String
	 * @param maxLength an int which determines the truncated length of the
	 *                  associated texts
	...
	public Tekst findById(Long textId) {
		return textRepo.findById(textId).orElseThrow(
				() -> new SearchTermNotFoundException("Tekst mit dieser ID inexistent " + "( " + textId + " )"));

	}

	/**
	 * searches for a Tekst by the given String
	 * 
	 * @param text - search String
	 * @return found Tekst or null with empty String
	 */
	public Tekst findByText(String text) {
		if (text == null || text.isBlank()) {
			return null;
		}
		return textRepo.findByText(text.strip());
	}

	/**
	 * searches for Teksts by the given fragment, and truncates them to the given
	 * length
	 * 
	...
	public void validateSearchString(String searchTerm) {
		if (searchTerm == null || searchTerm.isEmpty() || searchTerm.matches(WHITESPACE_REGEX)) {
			LOG.info("\n NO SearchTerm  ");
	...
	public List<Tag> findRandomTag(int x) {
		...
		while (uniqueTags.size() < x && attempts < maxAttempts) {
			...
			if (randomTag != null) {
				...
				LOG.info("\n SearchService.findRandomTag() --> Random Tag: " + randomTag.getId() + " / "
						+ randomTag.getTagText());
			}
	...
	public List<Author> findRandomAuthor(int x) {
		...
		while (uniqueAuthors.size() < x && attempts < maxAttempts) {
			...
			if (randomAuthor != null) {
				...
				LOG.info("\n SearchService.findRandomAuthor() --> Random Author: " + randomAuthor.getAuthorId() + " / "
						+ randomAuthor.getAuthorFamilyName());
			}
			attempts++;
		}
	...
	public Tag findTagById(Long tagId) {
		return tagRepo.findById(tagId).orElseThrow(() -> new TagNotFoundException("Tag not found with id " + tagId));
	}
